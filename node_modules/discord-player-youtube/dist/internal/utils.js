...
function normalizeCandidateToItem(candidate) {
    try {
        ...
        const video_id = candidate?.video_id || candidate?.id || candidate?.content_id || null;
        if (typeof video_id !== "string")
            return null;
        const title = candidate?.title?.text ||
            toStringSafe(candidate?.metadata?.title) ||
            candidate?.title ||
            "UNKNOWN TITLE";
        const author = candidate?.author?.name || "UNKNOWN AUTHOR";
        const thumb = candidate?.best_thumbnail?.url ||
...
function parseEnglishDurationLabel(input) {
    try {
        const s = String(input || "").toLowerCase();
        let total = 0;
        const re = /(\d+)\s*(hour|hours|hr|hrs|minute|minutes|min|mins|second|seconds|sec|secs)\b/g;
        let match;
...
export function buildTrack(player, item, ctx, playlist, extractor) {
    const normalized = normalizeCandidateToItem(item) ?? item;
    const durationTC = Util.buildTimeCode(Util.parseMS(normalized.durationMS || 0));
    const t = new Track(player, {
        ...
        requestedBy: ctx?.requestedBy ?? undefined,
        source: normalized.source ?? "youtube",
        queryType: "youtubeVideo",
        metadata: {
            source: normalized.raw ?? item,
            bridge: normalized?.raw?.bridge ?? null,
        },
...
export function buildPlaylistMeta(player, meta) {
...
export function msFromDurationLike(input) {
    try {
...
export function textFromRuns(obj) {
    try {
        const runs = obj?.runs;
        if (runs?.length)
            return String(runs.map((r) => r.text).join(""));
...
export function extractVideoId(input) {
    ...
    try {
        ...
        if (host === "youtu.be" || host.endsWith(".youtu.be")) {
            ...
            if (segs.length > 0 && idPattern.test(segs[0]))
                return segs[0];
        }
...
export async function searchYoutubeByQueryName(innertube, query) {
    let search;
    try {
...
export function checkIsUrl(query) {
    let isUrl;
    try {
        new URL(query);
        isUrl = true;
    ...
    return (isUrl ||
        new RegExp("^https?://(www\\.)?(youtube\\.com|youtu\\.be)/", "i").test(query));
}
